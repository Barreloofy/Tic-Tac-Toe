...
/// - Remark:
/// Use the static method 'prepare(_ :String)' to create an instance of AVAudioPlayer with the content of an audio-file.
...
class Sound: NSObject, AVAudioPlayerDelegate {
  @MainActor static let shared = Sound()
  ...
  func play(_ name: String, with delay: TimeInterval = 0.5) {
    guard let player = Sound.prepare(name) else { return }
